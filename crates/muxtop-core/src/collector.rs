...
impl Collector {
    ...
    pub fn with_engines(
        ...
        Self {
            ...
            // the environ/cwd/root/exe we deliberately skip below. Start
            // empty; `run` seeds the baseline with the targeted refresh.
        ...
        loop {
            tokio::select! {
                _ = interval.tick() => {
                    ...
                    // Both clones are refcount bumps (PERF-02), not deep copies.
...
mod tests {
    ...
    /// a refresh kind that collects nothing would also pass the assertions
    /// above.
    #[test]
